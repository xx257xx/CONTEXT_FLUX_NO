...
def select_kv(key, val, len_full, len_select, select_method):
    ...
    #     pass
        ...
        if select_method == "random":
            ...
            index = tf.random.experimental.stateless_shuffle(
                tf.range(len_full), seed=seed
            )[0:len_select]
...
def apply_random_demo_num_in_use(
    demo_num, config, demo_cond_mask_list, demo_qoi_mask_list
):
    """
    randomly select the number of demos to be used in the current prompt
    """
    demo_num_in_use = tf_rng_seq.uniform(
        shape=(),
        minval=config["demo_num_begin"],
        maxval=config["demo_num_end"],
        dtype=tf.int32,
    )
    demo_in_use_mask = tf.pad(
        tf.ones((demo_num_in_use), dtype=tf.int32), [[0, demo_num - demo_num_in_use]]
    )
...
def apply_cond_qoi_len_in_use(
    demo_num,
    cond_len,
    qoi_len,
    config,
    cond_len_in_use=None,
    qoi_len_in_use=None,
    demo_cond_mask_list=None,
    demo_qoi_mask_list=None,
    ...
    if cond_len_in_use is None:
        cond_len_in_use = tf_rng_seq.uniform(
            shape=(demo_num + 1,),
            minval=config["cond_len_in_use_begin"],
            maxval=config["cond_len_in_use_end"],
            dtype=tf.int32,
        )
    if qoi_len_in_use is None:
        qoi_len_in_use = tf_rng_seq.uniform(
            shape=(demo_num + 1,),
            minval=config["qoi_len_in_use_begin"],
            maxval=config["qoi_len_in_use_end"],
            dtype=tf.int32,
        )
    ...
    for i in range(demo_num):
        ...
        cond_len_mask_i = tf.pad(
            tf.ones((cond_len_in_use_i,), dtype=tf.int32),
            [[0, cond_len - cond_len_in_use_i]],
        )
        qoi_len_mask_i = tf.pad(
            tf.ones((qoi_len_in_use_i,), dtype=tf.int32),
            [[0, qoi_len - qoi_len_in_use_i]],
        )
    ...
    cond_len_mask_quest = tf.pad(
        tf.ones((cond_len_in_use_quest,), dtype=tf.int32),
        [[0, cond_len - cond_len_in_use_quest]],
    )
    qoi_len_mask_quest = tf.pad(
        tf.ones((qoi_len_in_use_quest,), dtype=tf.int32),
        [[0, qoi_len - qoi_len_in_use_quest]],
    )
...
def build_ode_forward(
    ...
    demo_cond_mask_list, demo_qoi_mask_list = apply_random_demo_num_in_use(
        demo_num, config, demo_cond_mask_list, demo_qoi_mask_list
    )
    return (
        equation,
...
def build_ode_inverse(
    ...
    demo_cond_mask_list, demo_qoi_mask_list, quest_cond_mask, quest_qoi_mask = (
        apply_cond_qoi_len_in_use(
            demo_num,
            cond_len,
            qoi_len,
            config,
            cond_len_in_use=cond_len_in_use,
            qoi_len_in_use=qoi_len_in_use,
        )
    )
    demo_cond_mask_list, demo_qoi_mask_list = apply_random_demo_num_in_use(
        demo_num, config, demo_cond_mask_list, demo_qoi_mask_list

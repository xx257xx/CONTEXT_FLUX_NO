...
class Runner:
    def __init__(
        ...
        self.params = jax.device_put_replicated(
            self.params, devices
        )  # replicate the params to all devices
        self.opt_state = jax.device_put_replicated(
            self.opt_state, devices
        )  # replicate the opt_state to all devices
        self.predict_pmap_batch_fn = jax.pmap(
            self.predict_batch_fn, axis_name="devices"
        )  # make predictions in batch and devices
        self.loss_pmap_batch_fn = jax.pmap(
            self.loss_batch_fn, axis_name="devices"
        )  # no average over batch and devices
        self.loss_pmap_batch_ave_fn = jax.pmap(
            self.loss_batch_ave_fn, axis_name="devices"
        )  # average over batch, no average over devices
    ...
    def save(self, save_dir):
        ...
        with open(params_path, "wb") as file:
            pickle.dump(
                jax.device_get(jax.tree_util.tree_map(lambda x: x[0], self.params)),
                file,
            )
        with open(opt_state_path, "wb") as file:
            pickle.dump(
                jax.device_get(jax.tree_util.tree_map(lambda x: x[0], self.opt_state)),
                file,
            )
    ...
    def iter(self, prompt, mask, query, query_mask, ground_truth, use_list=False):
        ...
        self.params, self.opt_state = train_iter(
            self.params,
            self.next_key(),
            self.opt_state,
            prompt,
    ...
    def get_loss(self, prompt, mask, query, query_mask, ground_truth, use_list=False):
        ...
        if use_list:
            losses = []
            for prompt_i, mask_i, query_i, query_mask_i, ground_truth_i in zip(
                prompt, mask, query, query_mask, ground_truth
            ):
                losses.append(
                    self.loss_pmap_batch_fn(
                        self.params,
                        self.next_key(),
                        prompt_i,
                        mask_i,
            ...
            losses = jnp.concatenate(
                losses, axis=0
            )  # (num_devices * list_size, batch_on_each_device)
        else:
    ...
    def get_pred(self, prompt, mask, query, use_list=False):
        ...
        if use_list:
            ...
            for (
                ...
                mask_i,
                query_i,
            ) in zip(prompt, mask, query):
                pred.append(
                    self.predict_pmap_batch_fn(
                        self.params, self.next_key(), prompt_i, mask_i, query_i
                    )
                )
...
def run_train():
    ...
    train_file_names = [
        "{}/{}".format(i, j) for i in train_data_dirs for j in FLAGS.train_data_globs
    ]
    test_file_names = [
        "{}/{}".format(i, j) for i in test_data_dirs for j in FLAGS.test_data_globs
    ]
    ...
    for _ in range(FLAGS.epochs * FLAGS.steps_per_epoch + 1):
        ...
        if runner.train_step % 100 == 0:  # print and write loss to tfboard
            ...
            _, prompt, mask, query, query_mask, ground_truth = train_data.get_next_data(
                list_size=FLAGS.list_size
            )
            this_train_loss = runner.get_loss(
                prompt,
                mask,
                query,
                query_mask,
                ground_truth,
                use_list=bool(FLAGS.list_size),
            )
            this_train_loss_mean = jnp.mean(this_train_loss)
            this_train_loss_std = jnp.std(this_train_loss)

            equation, prompt, mask, query, query_mask, ground_truth = (
                test_data.get_next_data(decode_equation=True, list_size=FLAGS.list_size)
            )
            this_test_loss = runner.get_loss(
                prompt,
                mask,
                query,
                query_mask,
            ...
            this_test_loss_mean = jnp.mean(this_test_loss)
            this_test_loss_std = jnp.std(this_test_loss)

            print(
                "step: {}, train loss: {:.4f}+-{:.4f}, test loss: {:.4f}+-{:.4f}".format(
                    runner.train_step,
                    this_train_loss_mean,
                    this_train_loss_std,
                    this_test_loss_mean,
                    this_test_loss_std,
                ),
            ...
            if FLAGS.tfboard:
                with file_writer.as_default():
                    tf.summary.scalar(
                        "loss/train_loss", this_train_loss_mean, step=runner.train_step
                    )
                    tf.summary.scalar(
                        "loss/test_loss", this_test_loss_mean, step=runner.train_step
                    )
        ...
        if runner.train_step % 1000 == 0:  # plot to tfboard
            ...
            pred = runner.get_pred(
                prompt, mask, query, use_list=bool(FLAGS.list_size)
            )  # (num_devices * list_size, batch_on_each_device, ...)
            if FLAGS.list_size:
...
if __name__ == "__main__":
    ...
    flags.DEFINE_integer(
        "list_size", 0, "size of list used to increase batch size, 0 means no"
    )
    ...
    flags.DEFINE_list(
        "train_data_globs", ["train*"], "filename glob patterns of training data"
    )
    flags.DEFINE_string(
    ...
    flags.DEFINE_string(
        "preprocess", "id", "the method for preprocessing keys and values"
    )
    ...
    flags.DEFINE_string(
        "initializer", "glorot_uniform", "the initializer for the transformers"
    )

...
class Runner():
  ...
  def save(self, save_dir):
    ...
    with open(params_path, 'wb') as file:
      pickle.dump(jax.device_get(jax.tree_util.tree_map(lambda x: x[0], self.params)), file)
    with open(opt_state_path, 'wb') as file:
      pickle.dump(jax.device_get(jax.tree_util.tree_map(lambda x: x[0], self.opt_state)), file)

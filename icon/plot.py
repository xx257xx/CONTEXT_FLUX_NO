...
def plot_all_in_one(
    ...
    axs[0].plot(
        prompt[mask_cond_quest, cond_k_index],
        prompt[mask_cond_quest, k_dim],
        "k+",
        markersize=7,
        label="cond quest",
    )
    ...
    axs[1].plot(
        query[query_mask, qoi_k_index],
        ground_truth[query_mask, 0],
        "k+",
        markersize=7,
        label="ground truth",
    )
    ...
    for i in range(demo_num):
        ...
        if np.sum(mask_cond_i) > 0 and np.sum(mask_qoi_i) > 0:  # demo that is used
            ...
            axs[0].plot(
                prompt[mask_cond_i, cond_k_index],
                prompt[mask_cond_i, k_dim],
                "o",
                markersize=3,
                label="cond {}".format(i),
                alpha=0.5,
            ...
            axs[1].plot(
                prompt[mask_qoi_i, qoi_k_index],
                prompt[mask_qoi_i, k_dim],
                "o",
                markersize=3,
                label="qoi {}".format(i),
                alpha=0.5,
    ...
    axs[0].set_title(
        "cond, {} demos mask nonzero num: {}, quest mask nonzero num: {}".format(
            cond_mask_nonzero_num.shape[0],
            cond_mask_nonzero_num,
            np.sum(mask_cond_quest),
        )
    )
    axs[1].set_title(
        "demo qoi, {} demos mask nonzero num: {}".format(
            qoi_mask_nonzero_num.shape[0], qoi_mask_nonzero_num
        )
    )
...
if __name__ == "__main__":
    ...
    for i in range(demo_num - 1):
        ...
        print(
            "the {}th demo cond k is {}, cond v is {}, qoi k is {}, qoi v is {}".format(
                i, demo_cond_k, demo_cond_v, demo_qoi_k, demo_qoi_v
            )
        )
    quest_cond = (np.arange(6)[:, None] * 10, np.arange(6)[:, None] * 10.1)
    prompt, mask = pad_and_concat(
        demos=demos,

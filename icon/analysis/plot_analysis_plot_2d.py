    ...
    for equation in results:
        ...
        for plot_index in range(0, plot_num * 10, 10):
            ...
            gs = gridspec.GridSpec(
                1, 7, width_ratios=[0.1, 0.22, 1, 1, 1, 1, 0.1]
            )  # Create a 1x5 grid of subplots

            # Subplot 1: condition
            ax1 = plt.subplot(gs[2])
            cond_k = this_result["raw_quest_cond_k"][plot_index, 0].reshape(
                (-1, x_size, 2)
            )  # [t_size, x_size, 2]
            cond_v = this_result["raw_quest_cond_v"][plot_index, 0].reshape(
                (-1, x_size)
            )  # [t_size, x_size]
            plt.pcolormesh(
                cond_k[..., 0],
            ...
            plt.plot(
                prompt[plot_index, mask_cond_i, cond_k_index - 1],
            ...
            t, x, v = sort_k_v(
                query[plot_index], pred[plot_index], k_dim, k_mode, x_size
            )
            plt.pcolormesh(t, x, v, shading=shading, vmin=vmin, vmax=vmax, cmap=rhocmap)
            plt.xlabel(r"$t$")
            plt.yticks([])
            ...
            t, x, v = sort_k_v(
                query[plot_index],
                pred[plot_index] - gt[plot_index],
                k_dim,
                k_mode,
                x_size,
            )
            plt.pcolormesh(
                t, x, v, shading=shading, vmin=evmin, vmax=evmax, cmap=errcmap
            )
            plt.xlabel(r"$t$")
            plt.yticks([])
            plt.title("question\n QoI error")
            ...
            plt.savefig(
                f"{analysis_folder}/{equation}_index{plot_index}_demo{demo_num}_question.png",
                dpi=600,
                bbox_inches="tight",
            )
            ...
            for demo_i in range(demo_num):
                ...
                cond_k = this_result["raw_demo_cond_k"][
                    plot_index, demo_i
                ]  # [cond_tx_size, 2]
                cond_v = this_result["raw_demo_cond_v"][
                    plot_index, demo_i
                ]  # [cond_tx_size, 1]
                qoi_k = this_result["raw_demo_qoi_k"][
                    plot_index, demo_i
                ]  # [qoi_tx_size, 2]
                qoi_v = this_result["raw_demo_qoi_v"][
                    plot_index, demo_i
                ]  # [qoi_tx_size, 1]
                cond_qoi_k = np.concatenate([cond_k, qoi_k], axis=0).reshape(
                    (-1, x_size, 2)
                )  # [t_size, x_size, 2]
                cond_qoi_v = np.concatenate([cond_v, qoi_v], axis=0).reshape(
                    (-1, x_size)
                )  # [t_size, x_size]
                plt.pcolormesh(
                    cond_qoi_k[..., 0],
                ...
                plt.plot(
                    prompt[plot_index, mask_qoi_i, cond_k_index - 1],
                    prompt[plot_index, mask_qoi_i, cond_k_index],
                    "o",
                    markersize=3,
                    color="red",
                    alpha=0.7,
                ...
                plt.tight_layout()
                plt.savefig(
                    f"{analysis_folder}/{equation}_index{plot_index}_demo{demo_num}_{demo_i}.png",
                    dpi=600,
                    bbox_inches="tight",
                )
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string(
        "analysis_folder",
        "../analysis/analysis0511a-v4-len-50-50-50-2600",

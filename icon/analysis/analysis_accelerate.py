...
def get_key(task, eqn_name):
    ...
    if task == "ind" or task == "len":
        ...
        if "ode_auto_const" in eqn_name:
            ...
            "ode_auto_linear1" in eqn_name
        ):  # e.g., ode_auto_linear1_forward_0.200_-0.091
            coeffs = eqn_name.split("_")
            # amins = np.linspace(0.1, 4, FLAGS.ood_coeff1_grids)[:-1]; gap_a = amins[1] - amins[0]
            # bmins = np.linspace(-3, 3, FLAGS.ood_coeff2_grids)[:-1]; gap_b = bmins[1] - bmins[0]
            # all_params.append("{:.3f}_{:.3f}".format(coeff_a, coeff_b))
            ...
            "ode_auto_linear2" in eqn_name
        ):  # e.g., ode_auto_linear2_forward_0.128_0.556_1.156
            coeffs = eqn_name.split("_")
            # amins = np.linspace(0.1, 4, FLAGS.ood_coeff1_grids)[:-1]; gap_a = amins[1] - amins[0]
            # bmins = np.linspace(-3, 3, FLAGS.ood_coeff2_grids)[:-1]; gap_b = bmins[1] - bmins[0]
            ...
            "pde_porous_spatial" in eqn_name
        ):  # e.g., pde_porous_spatial_forward_0.128_0.560_-5.248_0.404, four num: ul, ur, c, a
            coeffs = eqn_name.split("_")
            # amins = np.linspace(0.1, 4, FLAGS.ood_coeff1_grids)[:-1]; gap_a = amins[1] - amins[0]
            ...
            coeff1_buck = (
                int(np.floor(float(coeffs[7]) * 10)) / 10
            )  # coeffa, buck size 0.1
            coeff2_buck = (
                int(np.floor(float(coeffs[6]) * 2.5)) / 2.5
            )  # coeffc, buck size 0.4
            key = ("_".join(eqn_name.split("_")[:4]), coeff1_buck, coeff2_buck)
        elif (
            "series_damped_oscillator" in eqn_name
        ):  # e.g., series_damped_oscillator_forward_-0.624
            coeffs = eqn_name.split("_")
            ...
            coeff1_buck = (
                int(np.floor(float(coeffs[4]) * 6.66)) / 6.66
            )  # decay, buck size 0.15
...
def apply_operator(equation_list, prompt, fake=True):
    ...
    for i, this_eqn in enumerate(equation_list):
        ...
        cond_len = int(
            np.sum(prompt[i, 500:, -1])
        )  # int, length of the condition with mask = True
...
def rebuild_prompt(equation, prompt, mask, query, query_mask, ground_truth):
    ...
    assert (
        np.shape(prompt)[-2]
        == (FLAGS.cond_len + FLAGS.qoi_len) * FLAGS.demo_num + FLAGS.cond_len
    )
    assert (
        np.shape(mask)[-1]
        == (FLAGS.cond_len + FLAGS.qoi_len) * FLAGS.demo_num + FLAGS.cond_len
...
def write_errdict(
    errdict, equation, prompt, mask, query, query_mask, ground_truth, pred, raw
):
...
def get_errdict(data, runner, errdict, figdict, fake_demo_data=None):
    ...
    while True:
        ...
        read_step += 1
        try:
            raw, equation, prompt, mask, query, query_mask, ground_truth = (
                data.get_next_data(decode_equation=True, return_raw=True)
            )
        except StopIteration:
            break

        if fake_demo_data is not None:
            fake_equation, fake_prompt, fake_mask, _, _, _ = (
                fake_demo_data.get_next_data(decode_equation=True, return_raw=False)
            )
            ...
            prompt = jnp.concatenate(
                [fake_prompt[..., :demo_len, :], prompt[..., demo_len:, :]], axis=-2
            )
            mask = jnp.concatenate(
                [fake_mask[..., :demo_len], mask[..., demo_len:]], axis=-1
            )

        if FLAGS.task == "len":
            equation, prompt, mask, query, query_mask, ground_truth = rebuild_prompt(
                equation, prompt, mask, query, query_mask, ground_truth
            )
            if read_step == 1:
                print(
                    "prompt: ",
                    np.shape(prompt),
                    "sum: ",
                    np.sum(np.abs(prompt[..., (FLAGS.k_dim + FLAGS.v_dim) :])),
                    flush=True,
                )
        ...
        if FLAGS.mode == "fake_op":
            prompt, mask, query, query_mask, ground_truth = clean_data(
                prompt, mask, query, query_mask, ground_truth
            )
        ...
        plot_figure(
            figdict, equation, prompt, mask, query, query_mask, ground_truth, pred
        )
...
def main(argv):
    ...
    pprint(test_files)
    print(
        "===========test files length {}=========".format(len(test_files)), flush=True
    )
        ...
        for test_file in test_files:
            if "ode_auto_linear3_forward.tfrecord" in test_file:
                demo_file = [
                    i
                    for i in test_files
                    if "_0.00_" in i and "ode_auto_linear3_forward.tfrecord" in i
                ][0]
            elif "ode_auto_linear3_inverse.tfrecord" in test_file:
                demo_file = [
                    i
                    for i in test_files
                    if "_0.00_" in i and "ode_auto_linear3_inverse.tfrecord" in i
                ][0]
            else:
                raise NotImplementedError
            print("test_file: ", test_file, flush=True)
            print("demo_file: ", demo_file, flush=True)
            data = dataloader.DataProvider(
                seed=seed,
                demo_num=demo_num,
                cond_len=cond_len,
                qoi_len=qoi_len,
            ...
            fake_demo_data = dataloader.DataProvider(
                seed=seed,
                demo_num=demo_num,
                cond_len=cond_len,
                qoi_len=qoi_len,
    ...
    for key, value in errdict.items():  # key is the equation name
        ...
        print(
            "{} size {} & {:.5f}$\pm${:.5f} & {:.5f}$\pm${:.5f} \\\\".format(
                key,
                {k: value[k].shape for k in value.keys() if hasattr(value[k], "shape")},
                error_mean,
                2 * error_std,
                error_mean / gtmean,
                2 * error_std / gtmean,
...
if __name__ == "__main__":
    ...
    flags.DEFINE_enum(
        "mode",
        "vanilla",
        ["vanilla", "fake_demo", "fake_op", "real_op"],
        "mode of analysis",
    )
    ...
    flags.DEFINE_list(
        "test_data_globs", ["test*"], "filename glob patterns of testing data"
    )
    flags.DEFINE_string(
        "test_config_filename", "test_config.json", "config file for testing"
    )
    ...
    flags.DEFINE_integer(
        "len_demo_cond_len", 100, "the length of the demo condition for task len"
    )
    flags.DEFINE_integer(
        "len_demo_qoi_len", 100, "the length of the demo qoi for task len"
    )
    flags.DEFINE_integer(
        "len_quest_cond_len", 100, "the length of the quest condition for task len"
    )
    flags.DEFINE_integer(
        "len_quest_qoi_len", 2600, "the length of the quest qoi for task len"
    )
    ...
    flags.DEFINE_integer(
        "qoi_v_dim", 1, "the output dim of the model, also the dim of qoi_v "
    )

    # save_raw and save_prompt consume a lot of memory and storage, be careful
    flags.DEFINE_bool(
        "save_raw", False, "return raw data, be careful, see dataloader.py for details"
    )
    flags.DEFINE_bool(
        "save_prompt", False, "save equation names, prompts, masks and queries"
    )

...
def read_file(prob_type, analysis_folder, stamp, datadir, demo_num_begin, demo_num_end):
    with open(
        f"{analysis_folder}/err_{stamp}_{datadir}_{demo_num_begin}_{demo_num_end}.pickle",
        "rb",
    ) as file:
...
def plot_heatmap(
    ...
    print(
        f"title: {title}, {ood_coeff1_min}-{ood_coeff1_gap:.3f}-{ood_coeff1_max}, {ood_coeff2_min}-{ood_coeff2_gap:.3f}-{ood_coeff2_max}"
    )
    im = ax.pcolormesh(
        new_amins + ood_coeff1_gap / 2,
        new_bmins + ood_coeff2_gap / 2,
        new_err_mean,
        cmap="bwr",
        vmin=0,
        vmax=0.1,
    ...
    rect = patches.Rectangle(
        (in_dist_coeff1_min, in_dist_coeff2_min),
        in_dist_coeff1_len,
        in_dist_coeff2_len,
        linewidth=1,
        edgecolor="k",
        ...
        for datadir in datadirs_plot:
            ...
            if "ood_seriesdamped" in filename_fwd:
                ...
                plot_heatmap(
                    ...
                    xylabel[prob_type]["y"],
                    title_fwd,
                    in_dist_coeff1_range[prob_type],
                    in_dist_coeff2_range[prob_type],
                )

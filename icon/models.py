...
def pad_and_concat(demos, quest_cond, k_dim, v_dim, cond_len, qoi_len, demo_num):
    ...
    for i, (demo_cond_k, demo_cond_v, demo_qoi_k, demo_qoi_v) in enumerate(demos):
        ...
        this_slice = prompt[
            i * (cond_len + qoi_len) : i * (cond_len + qoi_len) + demo_cond_len, :
        ]
        ...
        this_slice = prompt[
            i * (cond_len + qoi_len) + cond_len : i * (cond_len + qoi_len)
            + cond_len
            + demo_qoi_len,
            :,
        ]
    ...
    this_slice = prompt[
        demo_num * (cond_len + qoi_len) : demo_num * (cond_len + qoi_len)
        + quest_cond_len,
        :,
    ]
...
class SolverModel(hk.Module):
    ...
    def __call__(self, prompt, mask, query):
        ...
        sys_embedding = self.encoder(
            kv_embedding, einshape("i->kji", mask, k=1, j=len(prompt))
        )
        ...
        out_embedding = self.decoder(
            ...
            key=sys_embedding,
            value=sys_embedding,
            mask=einshape("i->kji", mask, k=1, j=len(query)),
        )
...
if __name__ == "__main__":
    ...
    prompt_long, mask_long = pad_and_concat(
        demos=demos,
        quest_cond=quest_cond,
        k_dim=3,
        v_dim=4,

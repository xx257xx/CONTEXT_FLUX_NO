...
def build_feature(
    ...
    if k_mode == "naive":
        ...
        if tf.strings.regex_full_match(equation, "ode.*forward.*"):
            demo_cond_k = tf.stack(
                [demo_cond_k[..., 1], demo_cond_k[..., 0]], axis=-1
            )  # index first, then time
            demo_qoi_k = tf.pad(demo_qoi_k, [[0, 0], [0, 0], [1, 0]])  # add zero index
            quest_cond_k = tf.stack(
                [quest_cond_k[..., 1], quest_cond_k[..., 0]], axis=-1
            )  # index first, then time
            quest_qoi_k = tf.pad(
                quest_qoi_k, [[0, 0], [0, 0], [1, 0]]
            )  # add zero index
        elif (
            tf.strings.regex_full_match(equation, "ode.*inverse.*")
        ...
        pass
    demo_cond_k = tf.pad(
        demo_cond_k, [[0, 0], [0, 0], [0, k_dim - tf.shape(demo_cond_k)[-1]]]
    )
    demo_cond_v = tf.pad(
        demo_cond_v, [[0, 0], [0, 0], [0, v_dim - tf.shape(demo_cond_v)[-1]]]
    )
    demo_qoi_k = tf.pad(
        demo_qoi_k, [[0, 0], [0, 0], [0, k_dim - tf.shape(demo_qoi_k)[-1]]]
    )
    demo_qoi_v = tf.pad(
        demo_qoi_v, [[0, 0], [0, 0], [0, v_dim - tf.shape(demo_qoi_v)[-1]]]
    )
    quest_cond_k = tf.pad(
        quest_cond_k, [[0, 0], [0, 0], [0, k_dim - tf.shape(quest_cond_k)[-1]]]
    )
    quest_cond_v = tf.pad(
        quest_cond_v, [[0, 0], [0, 0], [0, v_dim - tf.shape(quest_cond_v)[-1]]]
    )
    quest_qoi_k = tf.pad(
        quest_qoi_k, [[0, 0], [0, 0], [0, k_dim - tf.shape(quest_qoi_k)[-1]]]
    )
    quest_qoi_v = tf.pad(
        quest_qoi_v, [[0, 0], [0, 0], [0, v_dim - tf.shape(quest_qoi_v)[-1]]]
    )
    if return_raw:
        return (
            raw,
...
def build_prompt_and_mask(
    ...
    for i in range(demo_num):
        demo_cond_i_index = tf.one_hot(
            i * tf.ones((cond_len,), dtype=tf.int32), demo_num + 1
        )  # [cond_len, demo_num + 1]
        demo_cond_i = tf.concat(
            [demo_cond_k[i], demo_cond_v[i], demo_cond_i_index], axis=-1
        )  # [cond_len, k_dim + v_dim + demo_num + 1]
        demo_qoi_i_index = -tf.one_hot(
            i * tf.ones((qoi_len,), dtype=tf.int32), demo_num + 1
        )  # [qoi_len, demo_num + 1]
        demo_qoi_i = tf.concat(
            [demo_qoi_k[i], demo_qoi_v[i], demo_qoi_i_index], axis=-1
        )  # [qoi_len, k_dim + v_dim + demo_num + 1]
        prompt_list.append(demo_cond_i)
        prompt_list.append(demo_qoi_i)

    quest_cond_index = tf.one_hot(
        demo_num * tf.ones((cond_len,), dtype=tf.int32), demo_num + 1
    )  # [cond_len, demo_num + 1]
    quest_cond = tf.concat(
        [quest_cond_k[0], quest_cond_v[0], quest_cond_index], axis=-1
    )  # [cond_len, k_dim + v_dim + demo_num + 1]
    prompt_list.append(quest_cond)
    prompt = tf.concat(
        prompt_list, axis=0
    )  # [demo_num * (cond_len + qoi_len) + cond_len, k_dim + v_dim + demo_num + 1]
    ...
    prompt_mask = tf.concat(
        prompt_mask, axis=0
    )  # [demo_num * (cond_len + qoi_len) + cond_len]

    # should be optional, since we will apply mask in the model, but I want to make it clear in prompt
    prompt = (
        prompt * tf.cast(prompt_mask, tf.float32)[:, None]
    )  # [demo_num * (cond_len + qoi_len) + cond_len, k_dim + v_dim + demo_num + 1]
...
def build_model_input(
    ...
    prompt, prompt_mask = build_prompt_and_mask(
        demo_cond_k,
        demo_cond_v,
        demo_qoi_k,
        demo_qoi_v,
...
def get_tf_dataset(
    ...
    filenames_dataset = tf.data.Dataset.list_files(
        file_names, seed=seed + 1, shuffle=shuffle_dataset
    )
    ...
    if select == "random":
        dataset = dataset.map(
            partial(random_select, demo_num=demo_num),
            num_parallel_calls=num_parallel_calls,
        )
    elif select == "ordered":
        dataset = dataset.map(
            partial(ordered_select, demo_num=demo_num),
            num_parallel_calls=num_parallel_calls,
        )
    else:
    ...
    dataset = dataset.map(
        partial(
            build_model_input, demo_num=demo_num, cond_len=cond_len, qoi_len=qoi_len
        ),
        num_parallel_calls=num_parallel_calls,
    )
...
class DataProvider:
    ...
    def get_next_data(self, decode_equation=False, list_size=0, return_raw=False):
        raw, equation, prompt, mask, query, query_mask, ground_truth = next(
            self.dataset
        )
        if decode_equation:
        ...
        if list_size > 0:  # actually never used...
            prompt = jnp.split(
                einshape(
                    "(ij)...->ij...", prompt.numpy(), i=self.num_devices * list_size
                ),
                list_size,
                axis=0,
            )
            mask = jnp.split(
                einshape(
                    "(ij)...->ij...", mask.numpy(), i=self.num_devices * list_size
                ),
                list_size,
                axis=0,
            )
            query = jnp.split(
                einshape(
                    "(ij)...->ij...", query.numpy(), i=self.num_devices * list_size
                ),
                list_size,
                axis=0,
            )
            query_mask = jnp.split(
                einshape(
                    "(ij)...->ij...", query_mask.numpy(), i=self.num_devices * list_size
                ),
                list_size,
                axis=0,
            )
            ground_truth = jnp.split(
                einshape(
                    "(ij)...->ij...",
                    ground_truth.numpy(),
                    i=self.num_devices * list_size,
                ),
                list_size,
                axis=0,
            )
            ...
            prompt = einshape(
                "(ij)...->ij...", prompt.numpy(), i=self.num_devices
            )  # (num_devices, batch_size, prompt_length, prompt_dim)
            mask = einshape(
                "(ij)...->ij...", mask.numpy(), i=self.num_devices
            )  # (num_devices, batch_size, prompt_length)
            query = einshape(
                "(ij)...->ij...", query.numpy(), i=self.num_devices
            )  # (num_devices, batch_size, query_length, key_dim)
            query_mask = einshape(
                "(ij)...->ij...", query_mask.numpy(), i=self.num_devices
            )  # (num_devices, batch_size, query_length)
            ground_truth = einshape(
                "(ij)...->ij...", ground_truth.numpy(), i=self.num_devices
    ...
    def pretty_print(self, equation, prompt, mask, query, query_mask, ground_truth):
        pprint(equation)
        print(
            "prompt size: {}".format(tree.tree_map(lambda x: x.shape, prompt)),
            flush=True,
        )
        print(
            "mask size: {}".format(tree.tree_map(lambda x: x.shape, mask)), flush=True
        )
        print(
            "query size: {}".format(tree.tree_map(lambda x: x.shape, query)), flush=True
        )
        print(
            "query_mask size: {}".format(tree.tree_map(lambda x: x.shape, query_mask)),
            flush=True,
        )
        print(
            "ground_truth size: {}".format(
...
if __name__ == "__main__":
    ...
    for _ in range(10):
        ...
        print(
            np.concatenate(
                [query[0, ...], ground_truth[0, ...], 10 * query_mask[0, ...][:, None]],
                axis=1,
            )
        )

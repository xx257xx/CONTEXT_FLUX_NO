...
transpose_fn = lambda list_of_trees: tree.tree_map(
    lambda *xs: jnp.array(list(xs)), *list_of_trees
)
...
def get_scheduled_adamw(
    peak_lr, end_lr, warmup_steps, decay_steps, gnorm_clip, weight_decay
):
...
def get_partial_optimizer(
    params, trainable_key_list, untrainable_key_list, optimizer=None
):
    def is_in_list(key):
        flag = "zero"
...
def get_train_iter_pmap(loss_batch_ave_fn, optimizer):
    ...
    def train_iter(params, rng_key, opt_state, *args, **kwargs):
        grads = jax.grad(loss_batch_ave_fn)(
            params, rng_key, *args, **kwargs
        )  # gradient on each device
        grads = jax.lax.pmean(
            grads, axis_name="devices"
        )  # average the gradients over devices
        updates, opt_state = optimizer.update(
            grads, opt_state, params
        )  # update on each device, with synchronized gradients
        params = optax.apply_updates(
            params, updates
        )  # update parameters on each device
        return params, opt_state
...
class DataSet:
    ...
    def add_to_buffer(self, new_data_list):
        if self.cpu:  # force using cpu to store data
            buffer = [
                jnp.concatenate(
                    [
                        jax.device_put(d, jax.devices("cpu")[0]),
                        jax.device_put(nd, jax.devices("cpu")[0]),
                    ],
                    axis=0,
                )[-self.buffer_cap :, ...]
            ...
            buffer = [
                jnp.concatenate([d, nd], axis=0)[-self.buffer_cap :, ...]
                for d, nd in zip(self.buffer, new_data_list)
            ]
    ...
    def build_data(self):
        ...
        print(
            "current buffer size: {}/{}, data size: {}".format(
                len(self.buffer[0]), self.buffer_cap, self.size
            ),
            flush=True,
        )
